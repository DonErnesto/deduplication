...
def minhash_signature_fromshingles(shinglesdata, numhashes=10):
    ...
    for j in range(numhashes):
        ...
        signature_array[:, j] = shinglesdata.apply(
            lambda shingles: _minhash_fromshingles(shingles, A, B, MAXSHINGLEHASH)
        )
    return signature_array


def pairwise_slow_minhash_comparison(
    signature_array, sim_threshold=0.5, index_names=None
):
    """
    ...
    if index_names is not None:
        assert len(index_names) == len(
            signature_array
        ), "length index names should correspond with signature_array"
...
def pairwise_minhash_comparison(signature_array, sim_threshold=0.5, index_names=None):
    """
    Returns a dictionary with indices for which the similarity of the
    minhash signatures exceed sim_threshold.
    ...
    dist_gen = pairwise_distances_chunked(signature_array, metric="hamming")
...
def pairwise_cosine_comparison(textdata, index_names=None, sim_threshold=0.5):
    ...
    cv = CountVectorizer(
        lowercase=False, binary=True, ngram_range=[1, 2], max_features=20000
    )
    oh_matrix = cv.fit_transform(textdata)

    # row-by-row iterator over distance matrix
    dist_gen = pairwise_distances_chunked(oh_matrix, metric="cosine", working_memory=0)
    for i, dist_row in enumerate(dist_gen):
        duplicates_idx = np.where(dist_row < (1 - sim_threshold))[1]
...
def lsh_for_signaturematrix(signature_matrix, n_bands, index_names=None):
    ...
    if index_names is not None:
        assert (
            len(index_names) == signature_matrix.shape[0]
        ), "length index names should correspond with signature_array"
    assert not signature_matrix.shape[1] % n_bands, (
    ...
    for i in range(n_bands):
        ...
        signature_band = signature_band.sum(
            axis=1
        )  # NB: some - I guess very small -risk of collisions here
...
def test_performance(plagiaries, similar_items):
    ...
    for s_key, s_values in similar_items.items():
        duplicates = [s_value for s_value in s_values if s_value > s_key]
    ...
    for p_key, p_values in plagiaries.items():
        if isinstance(p_values, list):
            n_true_duplicates += len(
                [p_value for p_value in p_values if p_value > p_key]
            )
        else:
            if p_values > p_key:
                n_true_duplicates += 1
    ...
    print(
        "Of {:d} true duplicate pairs, {:d} were correctly identified (recall: {:.1f})".format(
            n_true_duplicates, tp, tp / n_true_duplicates
        )
    )
    print(
        "Of {:d} detected duplicate pairs, {:d} were correctly identified (precision: {:.1f})".format(
...
def load_groundtruth(path="./data", numdocs=1000):
    ...
    duplicate.
    """
    assert str(numdocs) in ("100", "1000", "2500", "10000"), (
        "Only 100 " ", 1000, 2500 and 10000 allowed for numdocs"
    )
...
def run_minhash_experiment(textdata, plagiaries, num_hashes=10, n_grams=2):
    time_dict = {}
    print(
        "Running MinHASH analysis for {} documents, with {} hashes".format(
            len(textdata), num_hashes
        )
    )
    ...
    shinglesdata = shingles_from_series(
        textdata, n_grams=n_grams, hashfunc=murmurhash3_32
    )
    ...
    signature_matrix = minhash_signature_fromshingles(
        shinglesdata, numhashes=num_hashes
    )
    ...
    similar_items = pairwise_minhash_comparison(
        signature_matrix, sim_threshold=0.8, index_names=textdata.index
    )
    ...
    print(
        "{:.2f}sec total\t\t{:.2f}sec\t\t{:.2f}sec\t\t{:.2}sec".format(
...
def run_minhashLSH_experiment(
    textdata, plagiaries, num_hashes=20, n_grams=2, n_bands=5
):
    time_dict = {}
    print(
        "\n\nRunning MinHASH+LSH analysis for {} documents, with {} hashes".format(
            len(textdata), num_hashes
        )
    )
    ...
    t3 = time.perf_counter()
    similar_items = lsh_for_signaturematrix(
        signature_matrix, n_bands=n_bands, index_names=textdata.index
    )
...
if __name__ == "__main__":
    ...
    run_minhashLSH_experiment(
        textdata, plagiaries, num_hashes=100, n_grams=2, n_bands=10
    )
